...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 'var(--radius-input)', padding: '10px 14px', fontSize: 12,
    }}>
  ...
  return (
    <Panel style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 3, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Response Time</div>
          <div style={{ fontWeight: 600, fontSize: 14 }}>{monitorName}</div>
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--faint)' }}>LAST 50</div>
      </div>
      <ResponsiveContainer width="100%" height={140}>
        <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
          ...
          <Area type="monotone" dataKey="ms" name="Response" stroke="#00d97e" strokeWidth={1.5} fill="url(#grad)" connectNulls={false} dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </Panel>
  )
